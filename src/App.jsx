...
function trackEvent(name, params = {}) {
  const payload = { ...params, ts: new Date().toISOString() }
...
function trackOnMouseDown(name, params = {}) {
...
function LeadFormModal({ onClose }) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      trackEvent('lead_form_submit_success', { canal: form.canal, tipo: form.tipo || 'na' })
      setSent(true)
    } catch {
      trackEvent('lead_form_submit_error', { canal: form.canal, tipo: form.tipo || 'na' })
          ...
          <div className="px-6 sm:px-8 py-8 text-center">
            ...
            {form.canal === 'telegram' && (
              <a href={TG_BOT} target="_blank" rel="noopener noreferrer"
                onMouseDown={trackOnMouseDown('cta_telegram_click', { location: 'form_success' })}
            ...
            {!yaEnvio ? (
              <>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <a href={WA_BOT} target="_blank" rel="noopener noreferrer"
                    onMouseDown={trackOnMouseDown('cta_whatsapp_click', { location: 'hero' })}
              ...
              <>
                ...
                <div className="flex items-center gap-5 flex-wrap">
                  <a href={TG_BOT} target="_blank" rel="noopener noreferrer"
                    onMouseDown={trackOnMouseDown('cta_telegram_click', { location: 'hero' })}
    ...
    <section id="como-funciona" className="bg-black border-t border-white/6" ref={ref}>
      <div className={`max-w-5xl mx-auto px-6 py-20 transition-all duration-700 ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        ...
        <div className="mt-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-8 border-t border-white/6">
          ...
          <a href={WA_BOT} target="_blank" rel="noopener noreferrer"
            onMouseDown={trackOnMouseDown('cta_whatsapp_click', { location: 'how_it_works' })}
  ...
  return (
    <section className="bg-white border-t border-black/8" ref={ref}>
      <div className={`max-w-4xl mx-auto px-6 py-16 transition-all duration-700 ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        ...
        <a href={WA_BOT} target="_blank" rel="noopener noreferrer"
          onMouseDown={trackOnMouseDown('cta_whatsapp_click', { location: 'report_preview' })}
  ...
  return (
    <footer className="bg-black border-t border-white/6" ref={ref}>
      <div className={`max-w-4xl mx-auto px-6 py-14 flex flex-col md:flex-row md:items-center md:justify-between gap-8 transition-all duration-700 ${vis ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <a href={WA_BOT} target="_blank" rel="noopener noreferrer"
          onMouseDown={trackOnMouseDown('cta_whatsapp_click', { location: 'footer' })}
...
export default function App() {
  ...
  const openLeadForm = useCallback((source = 'unknown') => {
  ...
  return (
    <div className="min-h-screen bg-black">
      <Navbar onOpenForm={() => openLeadForm('navbar')} />
      <Hero onOpenForm={() => openLeadForm('hero')} />
      <SocialProofStrip />
      <IntelligenceStrip />
      <HowItWorks onOpenForm={() => openLeadForm('how_it_works')} />
      <TwoChannels onOpenForm={() => openLeadForm('plans')} />
      <AddonPacks onOpenForm={() => openLeadForm('addon_packs')} />
      <Testimonials />
      <Comparison onOpenForm={() => openLeadForm('comparison')} />
      <ReportPreview onOpenForm={() => openLeadForm('report_preview')} />
      <PricingCalculator onOpenForm={() => openLeadForm('pricing_calculator')} />
      <FAQ />
      <Footer onOpenForm={() => openLeadForm('footer')} />
